...
for i in range(41,-1,-1):
    ...
    for place in dataplace:
        ...
        test = fin_coor.replace('<outerBoundaryIs>','(').replace('</outerBoundaryIs>',')').replace('<innerBoundaryIs>',',(').replace('</Polygon><Polygon>','),(').replace('</innerBoundaryIs>',')').replace('<Polygon>','POLYGON (').replace('</Polygon>',')').replace(',)',')')
